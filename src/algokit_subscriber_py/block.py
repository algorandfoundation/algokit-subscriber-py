...
def block_response_to_block_data(response: bytes) -> BlockData:
    return cast(BlockData, msgpack.unpackb(response, strict_map_key=False, unicode_errors='surrogateescape'))
...
def get_blocks_bulk(context: dict[str, int], client: AlgodClient) -> list[BlockData]:
    ...
    for chunk in block_chunks:
        ...
        for round_num in chunk:
            response = client.algod_request("GET", f"/blocks/{round_num}", params={"format": "msgpack"}, response_format="msgpack")
