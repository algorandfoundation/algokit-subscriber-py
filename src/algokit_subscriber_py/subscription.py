...
from .types.subscription import BalanceChange, BalanceChangeRole, BlockMetadata, NamedTransactionFilter, SubscribedTransaction, TransactionFilter, TransactionSubscriptionParams, TransactionSubscriptionResult
from .types.transaction import AlgodOnComplete, IndexerOnComplete, TransactionType
from .utils import chunk_array, encode_address, logger

SearchForTransactions = dict[str, Any]


def deduplicate_subscribed_transactions(txns: list[SubscribedTransaction]) -> list[SubscribedTransaction]:
...
def has_emitted_matching_arc28_event(  # noqa: PLR0913
    ...
    return any(
        ...
        if any(e['event_prefix'] == base64.b16encode(log[:4].encode('utf-8', errors='surrogateescape')).lower().decode() for e in potential_events)
    )
...
def indexer_pre_filter(
    ...
    if subscription.get('note_prefix'):
        args['note_prefix'] = base64.b64encode(subscription['note_prefix'].encode()).decode()
    ...
    if subscription.get('max_amount') and (
        subscription['type'] == "pay" or (subscription.get('asset_id') and (subscription['min_amount'] or 0) > 0)
    ):
...
def indexer_pre_filter_in_memory(subscription: TransactionFilter) -> Callable[[TransactionResult], bool]:  # noqa: C901
    """
    ...
    def filter_transaction(t: TransactionResult) -> bool:  # noqa: C901, PLR0912
        ...
        if subscription.get('receiver'):
            if isinstance(subscription['receiver'], str):
                result = result and (
                    (t.get('asset-transfer-transaction', {}).get('receiver') == subscription['receiver']) or
                    (t.get('payment-transaction', {}).get('receiver') == subscription['receiver'])
                )
            else:
                result = result and (
                    (t.get('asset-transfer-transaction', {}).get('receiver') in subscription['receiver']) or
                    (t.get('payment-transaction', {}).get('receiver') in subscription['receiver'])
                )
        ...
        if subscription.get('app_id'):
            if isinstance(subscription['app_id'], int):
                result = result and (
                    t.get('created-application-index') == int(subscription['app_id']) or
                    t.get('application-transaction', {}).get('application-id') == int(subscription['app_id'])
                )
            else:
                result = result and (
                    ((t.get('created-application-index') or 0) in map(int, subscription['app_id'])) or
                    (t.get('application-transaction', {}).get('application-id', 0) in map(int, subscription['app_id']))
                )
        ...
        if subscription.get('asset_id'):
            if isinstance(subscription['asset_id'], int | float):
                ...
                result = result and (
                    t.get('created-asset-index') == asset_id or
                    t.get('asset-config-transaction', {}).get('asset-id') == asset_id or
                    t.get('asset-freeze-transaction', {}).get('asset-id') == asset_id or
                    t.get('asset-transfer-transaction', {}).get('asset-id') == asset_id
                )
            else:
                asset_ids = set(map(int, subscription['asset_id']))
                result = result and (
                    t.get('created-asset-index') in asset_ids or
                    t.get('asset-config-transaction', {}).get('asset-id') in asset_ids or
                    t.get('asset-freeze-transaction', {}).get('asset-id') in asset_ids or
                    t.get('asset-transfer-transaction', {}).get('asset-id') in asset_ids
                )

        if subscription.get('min_amount'):
            result = result and (
                (t.get('payment-transaction', {}).get('amount', 0) >= subscription['min_amount']) or
                (t.get('asset-transfer-transaction', {}).get('amount', 0) >= subscription['min_amount'])
            )

        if subscription.get('max_amount'):
            result = result and (
                (t.get('payment-transaction', {}).get('amount', 0) <= subscription['max_amount']) or
                (t.get('asset-transfer-transaction', {}).get('amount', 0) <= subscription['max_amount'])
            )
...
def has_balance_change_match(transaction_balance_changes: list[BalanceChange], filtered_balance_changes: list[dict[str, Any]] | None) -> bool:
    ...
    def check_single_change(actual_change: dict, change_filter: dict) -> bool:
        # Address check
        address_check = (
            not change_filter.get('address') or
            (isinstance(change_filter.get('address'), list) and len(change_filter['address']) == 0) or
            (actual_change.get('address') in (change_filter['address'] if isinstance(change_filter.get('address'), list) else [change_filter.get('address')]))
        )

        # Minimum absolute amount check
        min_abs_amount_check = (
            change_filter.get('min_absolute_amount') is None or
            abs(actual_change.get('amount', 0)) >= change_filter['min_absolute_amount']
        )

        # Maximum absolute amount check
        max_abs_amount_check = (
            change_filter.get('max_absolute_amount') is None or
            abs(actual_change.get('amount', 0)) <= change_filter['max_absolute_amount']
        )

        # Minimum amount check
        min_amount_check = (
            change_filter.get('min_amount') is None or
            actual_change.get('amount', 0) >= change_filter['min_amount']
        )

        # Maximum amount check
        max_amount_check = (
            change_filter.get('max_amount') is None or
            actual_change.get('amount', 0) <= change_filter['max_amount']
        )

        # Asset ID check
        asset_id_check = (
            change_filter.get('asset_id') is None or
            (isinstance(change_filter.get('asset_id'), list) and len(change_filter['asset_id']) == 0) or
            (actual_change.get('asset_id') in (change_filter['asset_id'] if isinstance(change_filter.get('asset_id'), list) else [change_filter.get('asset_id')]))
        )

        # Role check
        role_check = (
            change_filter.get('role') is None or
            (isinstance(change_filter.get('role'), list) and len(change_filter['role']) == 0) or
            any(r in actual_change.get('roles', []) for r in (change_filter['role'] if isinstance(change_filter.get('role'), list) else [change_filter.get('role')]))
        )
    ...
    return any(
        any(check_single_change(actual_change, change_filter)
...
def get_subscribed_transactions(  # noqa: C901, PLR0912, PLR0915
    ...
    arc28_events: list[Arc28EventToProcess] = [
        {
            ...
            "event_prefix": hashlib.new('sha512_256', f"{e['name']}({','.join(a['type'] for a in e['args'])})".encode()).digest().hex()[:8],
            "event_definition": e
        }
    ...
    if current_round - watermark > max_rounds_to_sync:
        if sync_behaviour == 'fail':
            ...
            if subscription.get('max_indexer_rounds_to_sync') and indexer_sync_to_round_number - start_round + 1 > subscription['max_indexer_rounds_to_sync']:
                indexer_sync_to_round_number = start_round + subscription['max_indexer_rounds_to_sync'] - 1
    ...
    if not skip_algod_sync:
        ...
        algod_transactions = [
            ...
            if transaction_filter(f['filter'], arc28_events, subscription.get('arc28_events') or [])(t)
        ]
    ...
    if 'payment-transaction' in transaction:
        ...
        if close_to:
            changes.append({
                ...
                'asset_id': 0,
                'roles': [BalanceChangeRole.Sender, BalanceChangeRole.CloseTo],
            })
...
def get_filtered_indexer_transactions(transaction: TransactionResult, txn_filter: NamedTransactionFilter) -> list[SubscribedTransaction]:
    ...
    return list(filter(indexer_pre_filter_in_memory(txn_filter["filter"]), transactions)) # type: ignore[arg-type]
...
def indexer_post_filter(  # noqa: C901
    ...
    def filter_function(t: TransactionResult) -> bool:  # noqa: C901
        result = True
        ...
        if bool(subscription.get('app_on_complete')) and bool(t.get('application-transaction')):
            result &= t['application-transaction']['on-completion'].value == subscription.get('app_on_complete')

        if subscription.get('method_signature'):
            method_signature = subscription['method_signature']
            if isinstance(method_signature, str):
                result &= (
                    bool(t.get('application-transaction')) and
                    bool(t['application-transaction'].get('application-args')) and
                    t['application-transaction']['application-args'][0] == get_method_selector_base64(method_signature)
                )
            else:
                result &= any(
                    bool(t.get('application-transaction')) and
                    bool(t['application-transaction'].get('application-args')) and
                    t['application-transaction']['application-args'][0] == get_method_selector_base64(method)
        ...
        if subscription.get('app_call_arguments_match'):
            result &= (
                bool(t.get('application-transaction')) and
                subscription['app_call_arguments_match'](
                    [bytes.fromhex(a) for a in t['application-transaction'].get('application-args', [])]
                )
            )
        ...
        if subscription.get('arc28_events'):
            result &= (
                bool(t.get('application-transaction')) and
                bool(t.get('logs')) and
                has_emitted_matching_arc28_event(
                    t['logs'],
                    arc28_events,
                    arc28_event_groups,
                    subscription['arc28_events'],
                    t.get('created-application-index') or
                    t.get('application-transaction', {}).get('application-id', 0) or 0,
                    lambda: t
                )
            )
...
def transaction_filter(  # noqa: C901, PLR0915
    ...
    def filter_function(txn: TransactionInBlock) -> bool:  # noqa: C901, PLR0912, PLR0915
        t = txn['transaction'].dictify() # type: ignore[no-untyped-call]
        created_app_id, created_asset_id, logs = txn['created_app_id'], txn['created_asset_id'], txn['logs']
        result: bool = True
        ...
        if subscription.get('app_id'):
            ...
            if isinstance(app_id, int | float):
                ...
                result = result and bool((t.get('apid') and t['apid'] in app_ids) or
                                     bool(created_app_id and created_app_id in app_ids))
        ...
        if subscription.get('asset_id'):
            ...
            if isinstance(asset_id, int):
                ...
                result = result and bool((t.get('xaid') and t['xaid'] in asset_ids) or
                                     bool(created_asset_id and created_asset_id in asset_ids))
        ...
        if subscription.get('arc28_events'):
            result = result and (
                t.get('type') == TransactionType.appl.value and
                logs is not None and
                has_emitted_matching_arc28_event(
                    logs, arc28_events, arc28_event_groups, subscription['arc28_events'],
