...
from .types.subscription import AlgorandSubscriberConfig, TransactionSubscriptionParams, TransactionSubscriptionResult
...
class AlgorandSubscriber:
    ...
    def default_error_handler(self, error: Any, _str: str | None = None) -> None:  # noqa: ANN401
        raise error
    ...
    def poll_once(self) -> TransactionSubscriptionResult:
        ...
        self.event_emitter.emit('before:poll', {
            'watermark': watermark,
            'current_round': current_round
        })

        poll_result = get_subscribed_transactions(subscription=cast(TransactionSubscriptionParams, {
            'watermark': watermark,
            'current_round': current_round,
            **self.config
        }), algod=self.algod, indexer=self.indexer)

        try:
            for filter_name in self.filter_names:
                mapper = next((f.get('mapper') for f in self.config['filters'] if f['name'] == filter_name), None)
                matched_transactions = [t for t in poll_result['subscribed_transactions'] if filter_name in t.get('filters_matched', [])]
