...
# In this case, we are starting from the round that we know this address
# started sending transactions
...
def get_watermark() -> int:
    ...
    We are using the filesystem to ensure the value is persisted in the event
    of failures, power outages, etc.
    """
...
def set_watermark(new_watermark: int) -> None:
    """
    Write the new transactions to the CSV file and then write the new watermark
    value to the watermark file. This order is important to ensure we are not
    increasing the watermark value if we fail to write the transactions to the
    CSV file.
    """
...
subscriber = sub.AlgorandSubscriber(
    ...
    config=sub.AlgorandSubscriberConfig(
        ...
        watermark_persistence=sub.WatermarkPersistence(get=get_watermark, set=set_watermark),
...
def record_transaction(transaction: sub.SubscribedTransaction, _: str) -> None:
    """
    ...
    * The filter name (from the 'filters' list) that the transaction matched
    ...
    for bc in transaction.balance_changes:
        balance_changes.append(
            (
                transaction.confirmed_round or 0,
                transaction.id_,
                bc.asset_id,
                bc.amount,
            )
